<template>
  <div class="p-4">
    ...
    <div v-else>
      <table v-if="filteredTrips.length" class="min-w-full border border-gray-300 table-auto">
        <thead class="bg-gray-100">
          <tr>
            ...
            <th v-if="hasRole(['admin', 'manager'])" class="p-2 border">Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="trip in filteredTrips" :key="trip.id" class="hover:bg-gray-50">
            ...
            <td v-if="hasRole(['admin', 'manager'])" class="p-2 space-x-2 border">
              <router-link :to="`/trips/${trip.id}/edit`" class="text-blue-600 hover:underline">
                Edit
              </router-link>
              <button @click="deleteTrip(trip.id)" class="text-red-600 hover:underline">
                Delete
              </button>
            </td>
          </tr>
      ...
      <div v-else class="py-10 text-center text-gray-500">No trips found.</div>

      <!-- Pagination -->
      <div class="flex flex-wrap items-center justify-center mt-4 space-x-2 text-sm">
        <button @click="prevPage" :disabled="!pagination.prev_page_url" class="btn-pagination">
          Previous
        </button>
        <button
          v-for="p in pages"
        ...
        <button @click="nextPage" :disabled="!pagination.next_page_url" class="btn-pagination">
          Next
        </button>
      </div>
    </div>

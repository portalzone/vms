<template>
  <div class="p-4">
    <h2 class="mb-4 text-2xl font-semibold">Audit Trail</h2>
    ...
    <div class="mb-4">
      <div class="flex flex-wrap items-center gap-3 mb-4">
        ...
        <select v-model="selectedModule" class="px-3 py-2 border rounded">
          <option value="all">All Modules</option>
          <option v-for="name in moduleOptions" :key="name" :value="name">{{ name }}</option>
        </select>

        <select v-model="selectedTimeRange" class="px-3 py-2 border rounded">
          <option value="all">All Time</option>
          <option value="24h">Last 24 Hours</option>
          ...
        </select>

        <select v-model="perPage" class="px-3 py-2 border rounded">
        ...
        <div v-if="selectedTimeRange === 'custom'" class="flex gap-2">
          <input type="date" v-model="startDate" class="px-2 py-1 border rounded" />
          <input type="date" v-model="endDate" class="px-2 py-1 border rounded" />
        </div>
      </div>
      ...
      <table class="audit-table">
        <thead>
          <tr>
            ...
            <th class="p-3 text-right border">View</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="log in logs.data" :key="log.id" class="audit-table-row">
    ...
    <div class="flex flex-wrap items-center justify-center gap-2 mt-6 text-sm">
      <button :disabled="page === 1" @click="page-- && fetchLogs()" class="btn-pagination">
        Prev
      </button>
      ...
      <button :disabled="page === totalPages" @click="page++ && fetchLogs()" class="btn-pagination">
        Next
      </button>
    </div>
    ...
    <div v-if="selectedLog" class="modal-overlay" @click.self="selectedLog = null">
      <div class="modal-content overflow-y-auto max-h-[90vh] relative">
        ...
        <div class="space-y-2 text-sm text-gray-800 modal-body">
          ...
          <div>
            ...
            <pre class="p-2 overflow-auto text-xs bg-gray-100 rounded">{{
              formatJSON(selectedLog.properties?.old)
            }}</pre>
          </div>
          <div>
            <strong>New Values:</strong>
            <pre class="p-2 overflow-auto text-xs bg-gray-100 rounded">{{
              formatJSON(selectedLog.properties?.attributes)
            }}</pre>
          </div>
        </div>
      </div>
